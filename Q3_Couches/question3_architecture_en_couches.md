...
### 1. Couche Présentation *(Presentation / Interface / UI / View)*
...
**Technologies typiques :** fenêtres GUI, rapports imprimés, interfaces vocales, HTML/CSS/JavaScript, XML/XSLT, JSP.

---

### 2. Couche Application *(Application / Workflow / Process / App Controller)*
...
**Exemples de responsabilités :** gestion des sessions utilisateur, navigation entre les écrans, orchestration des appels vers le domaine, validation du flux de traitement.

---

### 3. Couche Domaine *(Domain / Business / Business Services / Model)*
...
Ce qui est important : ces règles peuvent parfois être partagées entre plusieurs applications (un service de gestion des stocks peut servir à la fois l'application de vente et l'application de logistique), mais elles restent spécifiques au domaine métier de l'entreprise.

**Exemples :** calcul du prix d'une commande, vérification de la disponibilité d'un article, règles de validation d'un paiement, gestion d'un point de vente (POS).

---

### 4. Couche Infrastructure Métier *(Business Infrastructure / Low-level Business Services)*
...
La différence avec la couche Domaine est subtile mais réelle : là où le Domaine contient des règles spécifiques à *ce* système (ex. les règles de tarification d'une location de voiture), l'Infrastructure Métier contient des services réutilisables dans n'importe quel contexte commercial (ex. la conversion de devises, le calcul de taxes standard, la gestion d'un calendrier).

**Exemples :** convertisseur de devises (`CurrencyConverter`), calculateur de TVA, gestion des fuseaux horaires, formatage d'adresses postales.

---

### 5. Couche Services Techniques *(Technical Services / Technical Infrastructure / High-level Technical Services)*
...
**Exemples :** couche de persistance (ORM, accès base de données), gestion de la sécurité (authentification, autorisation), moteur de règles, cache distribué, gestion des transactions.

---

### 6. Couche Fondation *(Foundation / Core Services / Base Services / Low-level Technical Services)*
...
**Exemples :** structures de données (listes, arbres, files), gestion des threads, opérations mathématiques, accès aux fichiers, entrées/sorties réseau, accès aux bases de données au niveau le plus bas (JDBC par exemple).
...
Le principe de dépendance unidirectionnelle (une couche ne dépend que de la couche en dessous) limite mécaniquement le couplage. La couche Présentation ne connaît pas l'existence de la couche Fondation — elle ne peut donc pas en dépendre directement. Ce cloisonnement réduit le nombre de relations entre composants et rend le système plus stable : une modification en bas de la pile ne peut se propager vers le haut que via les interfaces définies, pas de façon incontrôlée.
